...
export const requestEmailVerification = async (email: string): Promise<void> => {
  ...
  try {
    ...
    const url = `https://34.47.126.125:3000/api/auth/verify-email?token=${emailToken}`;
...
export const requestPasswordService = async (email: string): Promise<void> => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: '비밀번호 재설정',
      text: `비밀번호를 재설정하려면 링크를 클릭하세요 (유효시간: 3분): https://34.47.126.125:3000/reset-password?token=${token}`,
    };
    await transporter.sendMail(mailOptions);
